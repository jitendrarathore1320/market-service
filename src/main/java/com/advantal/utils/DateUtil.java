...
//			System.out.println("date 2 : "+strDateTime);
		}
		return strDateTime;
	}
	...
	public static Date convertStringToDateTimeYYYY(String date) {
		try {
			if (date != null) {
				...
				Date dateTime = dateFormat.parse(date);
//				System.out.println("date is : "+ dateTime.toString());
				return dateTime;
			} else {
				return null;
			}
...
//			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
	...
	public static String getPreviousDateTimes(String dateTime, String interval) {
		try {
			DateTimeFormatter formate = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
			LocalDateTime localdateTime = LocalDateTime.parse(dateTime, formate);
			if (interval.equalsIgnoreCase(Constant.ONE_DAY))
				localdateTime = localdateTime.minusHours(24);
			else if (interval.equalsIgnoreCase(Constant.ONE_WEEK))
				localdateTime = localdateTime.minusDays(7);
			else if (interval.equalsIgnoreCase(Constant.ONE_MONTH))
				localdateTime = localdateTime.minusDays(30);
			else if (interval.equalsIgnoreCase(Constant.SIX_MONTH))
				localdateTime = localdateTime.minusDays(179);
			else if (interval.equalsIgnoreCase(Constant.ONE_YEAR))
				localdateTime = localdateTime.minusDays(364);
			else if (interval.equalsIgnoreCase(Constant.FIVE_YEAR))
				localdateTime = localdateTime.minusDays(1824);
//			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
			return localdateTime.format(formatter);
...
//			String inputDateTimeString = "10/10/2023 08:41:58";
	...
	// Function to convert relative time expressions to specific date format
	public static String convertRelativeTime(String relativeTime) {
		LocalDateTime now = LocalDateTime.now();
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

		if (relativeTime.endsWith("minutes ago")) {
			int minutesAgo = Integer.parseInt(relativeTime.split(" ")[0]);
			LocalDateTime result = now.minusMinutes(minutesAgo);
			return result.format(formatter);
		} else if (relativeTime.endsWith("h ago")) {
			int hoursAgo = Integer.parseInt(relativeTime.split("")[0]);
			LocalDateTime result = now.minusHours(hoursAgo);
			return result.format(formatter);
		} else {
			try {
				String[] parts = relativeTime.split(" ");
				int dayOfMonth = Integer.parseInt(parts[1].replace(",", ""));
				...
				LocalDateTime result = LocalDateTime.of(year, month, dayOfMonth, 0, 0);
				return result.format(formatter);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
...
//	}
	...
	public static long previousFiveYearDay() {
		...
		// Calculate the total days between current date and five years ago
		long totalDays = ChronoUnit.DAYS.between(fiveYearsAgo, currentDate);

		System.out.println("Current Date: " + currentDate);
		System.out.println("Five Years Ago: " + fiveYearsAgo);
		System.out.println("Total Days in the Last 5 Years: " + totalDays);
		return totalDays;
	}

	public static int MinutesInADay() {
//		int hoursInADay = 24; // Hours in a day
//		int minutesInAnHour = 60; // Minutes in an hour

		int minutesInADay = 24 * 60;
		System.out.println("Minutes in a day: " + minutesInADay);
		return minutesInADay;
	}

	public static long MinutesInAWeek() {
//		int daysInAWeek = 7; // Number of days in a week
		long minutesInAWeek = Duration.ofDays(7).toMinutes();
		System.out.println("Minutes in a week: " + minutesInAWeek);
		return minutesInAWeek;
	}

	public static int HoursInAMonth() {
//		int year = Year.now().getValue(); // Change this to the desired year
//		int month = 4; // Change this to the desired month (1 for January, 2 for February, and so on)

		YearMonth yearMonth = YearMonth.of(Year.now().getValue(), 4);
//		int daysInMonth = yearMonth.lengthOfMonth(); // Number of days in the month
//		int hoursInADay = 24; // Hours in a day

		int hoursInMonth = yearMonth.lengthOfMonth() * 24;
		System.out.println("Hours in " + yearMonth.getMonth() + " " + Year.now().getValue() + ": " + hoursInMonth);
		return hoursInMonth;
	}

	public static long HoursInAThreeMonth(int month) {
		// Get the current year and month
		YearMonth currentYearMonth = YearMonth.now();

		// Calculate the end month
		YearMonth endMonth = currentYearMonth.plusMonths(month);

		// Calculate the total number of hours in the next three months
		long totalHours = ChronoUnit.HOURS.between(currentYearMonth.atDay(1).atStartOfDay(),
				endMonth.atEndOfMonth().atStartOfDay());

		System.out.println("Total hours in the next three months: " + totalHours);
		return totalHours;
	}

	public static long HoursInAOneYear() {
		// Get the current year
		Year currentYear = Year.now();

		// Calculate the start and end of the year
		LocalDate startOfYear = LocalDate.of(currentYear.getValue(), Month.JANUARY, 1);
		LocalDate endOfYear = LocalDate.of(currentYear.getValue(), Month.DECEMBER, 31);

		// Calculate the total number of hours in the year
		long totalHours = ChronoUnit.HOURS.between(startOfYear.atStartOfDay(), endOfYear.plusDays(1).atStartOfDay());

		System.out.println("Total hours in the current year: " + totalHours);
		return totalHours;
	}

	public static String Previous32DaysDateTime(String dateString) {
		// String representation of the date
//        String dateString = "2024-02-08 15:45:00";

		// Define the date formatter to parse the string
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

		// Parse the string into LocalDateTime
		LocalDateTime dateTime = LocalDateTime.parse(dateString, formatter);

		// Subtract 32 days from the parsed date and time
		LocalDateTime previousDateTime = dateTime.minusDays(32);

		// Print the previous date and time
		System.out.println("Previous date and time: " + previousDateTime.format(formatter));
		return previousDateTime.format(formatter);
	}

	public static int currentDateToPreviousDateCountInMonth() {
		// Get the current date
		LocalDate currentDate = LocalDate.now();
		...
		// Print the previous days within the current month
		System.out.println("Previous days in the current month:");
		int count = 1;
...
//        count = count + 1;
	...
	public static int currentDateToCountWeekendCountInMonth() {
		LocalDate currentDate = LocalDate.now();
		int currentDayOfMonth = currentDate.getDayOfMonth();

		// Initialize a counter for the number of weekends
		int weekendCount = 0;
		int count = currentDateToPreviousDateCountInMonth();
		// Iterate over each day in the current month up to the current date
		for (int i = 1; i <= currentDayOfMonth; i++) {
			LocalDate currentIterationDate = LocalDate.of(currentDate.getYear(), currentDate.getMonth(), i);
			// Check if the day falls on a weekend (Saturday or Sunday)
			if (currentIterationDate.getDayOfWeek() == DayOfWeek.SATURDAY
					|| currentIterationDate.getDayOfWeek() == DayOfWeek.SUNDAY) {
				weekendCount++;
			}
		}

		// Print the number of weekends
		System.out.println("Number of weekends in the current month up to the current date: " + weekendCount);
		weekendCount = count - weekendCount;
		System.out.println(
				"Number of days in the current month up to the current date with weekends days: " + weekendCount);
		return weekendCount;
	}

	public static long startToEndTimeConvertHours(String startTimeStr, String endTimeStr) {
		// Parse the start time and end time strings into LocalTime objects
		...
		// Adjust the end time if it's before the start time (e.g., crosses midnight)
		if (endTime.isBefore(startTime)) {
			endTime = endTime.plusHours(12); // Add 12 hours to the end time
		}
		...
		// Print the result
		System.out.println("The difference between " + startTimeStr + " and " + endTimeStr + " is: "
