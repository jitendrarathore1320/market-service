...
public class ExportUsCsvToPdf {

	List<CsvUsaExportFileRespnse> shariaComplianceList;

	public ExportUsCsvToPdf(List<CsvUsaExportFileRespnse> shariaComplianceList) {
...
//		table.addCell(cell);
	...
	private void writeTableData(PdfPTable table) {
		...
		for (CsvUsaExportFileRespnse shariaCompliance : shariaComplianceList) {
			PdfPCell cell = new PdfPCell();
			i = i + 1;
...
//			table.addCell(cell);
			...
			if (shariaCompliance.getInterestBearingSecuritiesAndAssetsStatus() != null
				...
				cell.setPhrase(new Phrase(shariaCompliance.getInterestBearingSecuritiesAndAssetsStatus(), font));
				table.addCell(cell);
			} else {
				cell.setPhrase(new Phrase("0", font));
				table.addCell(cell);
			...
			if (shariaCompliance.getInterestBearingSecuritiesAndAssetsPercentage() != null) {
				cell.setPhrase(new Phrase(shariaCompliance.getInterestBearingSecuritiesAndAssetsPercentage().toString(), font));
				table.addCell(cell);
			} else {
				cell.setPhrase(new Phrase("0", font));
				table.addCell(cell);
...
//		log.info("<<<<<<<<<< Added >>>> image in document  >>>>>> : " + document);
		...
		Paragraph p = new Paragraph("List of United States Sharia Compliance Details", font);
