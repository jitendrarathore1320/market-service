...
//	Page<Stock> findAllStocks(String country, String exchange, Pageable pageable);
	...
	@Query(value = "SELECT * FROM stock st WHERE st.country =?1 and st.index_name=?2 and st.status=1 and st.price!=0", countQuery = "SELECT count(*) from stock st WHERE st.country =?1 and st.index_name=?2  and st.status=1 and st.price!=0", nativeQuery = true)
	Page<Stock> getSaudiStockListByIndex(String country, String exchange, Pageable pageable);
